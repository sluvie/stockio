...
# Get Median, Typical, Weighted price
...
def iATR(df, ma_period, ma_method='SMA'):
    TR = np.max(np.vstack((df['High'].values, shift(df['Close'].values))).T, axis=1)\
        - np.min(np.vstack((df['Low'].values,
                 shift(df['Close'].values))).T, axis=1)
...
def iMACD(df, fast_period, slow_period, signal_period, applied_price='Close'):
    price = df[applied_price].values
    Main = MAonArray(price, fast_period, 'EMA') - \
        MAonArray(price, slow_period, 'EMA')
...
def iFrAMA(df, ma_period, ma_shift=0, applied_price='Close'):
    ...
    N2 = (H.shift(ma_period).rolling(ma_period).max() -
          L.shift(ma_period).rolling(ma_period).min())/ma_period
    N3 = (H.rolling(2*ma_period).max() -
          L.rolling(2*ma_period).min())/(2*ma_period)
...
def iADX(df, adx_period):
    ...
    Main = MAonArray(100*np.abs(PlusDI-MinusDI) /
                     (PlusDI+MinusDI), adx_period, 'EMA')
...
def iADXWilder(df, adx_period):
    ...
    Main = MAonArray(100*np.abs(PlusDI-MinusDI) /
                     (PlusDI+MinusDI), adx_period, 'SMMA')
    return pd.DataFrame({'Main': Main, 'PlusDI': PlusDI, 'MinusDI': MinusDI},
                        columns=['Main', 'PlusDI', 'MinusDI'], index=df.index)
...
def iIchimoku(df, tenkan_period, kijun_period, senkoub_period):
    ...
    SenkouB = (high.rolling(senkoub_period).max() +
               low.rolling(senkoub_period).min()).shift(kijun_period)/2
